...
def multiCrossValidation(X, y, k = 10):
    ...
    size = np.size(X, 1)
    identityMatrix = np.identity(size)
    # with I[1,1] = 0 if BETA is not regularized
    identityMatrix[0, 0] = 0
    rigidRegressionBeta = mdot( inv( dot(X.T, X) + (bestLambda * identityMatrix) ), X.T, y)
    
    X_grid = prependQuadraticFeatures(grid2d(-3, 3,num=30))
    
    # Predict with trained model
    # Here we calculate the f(x) ?, in slides it's f(x) = fi.Transpose * Beta, shouldn't then we take transpose of X_grid?
    y_grid = mdot(X_grid, rigidRegressionBeta)
